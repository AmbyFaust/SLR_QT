...
class CoordinateSystemEpsg(Enum):
    ...
    gauss_kruger = -1     # ФИКТИВНЫЙ ИДЕНТИФИКАТОР. Перевод в ГСК осуществляется особым способом
...
def translate_coordinates(src_crs: typing.Union[CoordinateSystemEpsg, int],
    ...
    Работа с проекцией Гаусса-Крюгера осуществляется особым способом (перевод можно осуществлять только в проекцию, не наоборот + отдельная функция для перевода)
    """
