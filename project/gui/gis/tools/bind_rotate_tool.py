...
MOVABLE_POINTS_COLOR = QColor(QtCore.Qt.green)
BASE_POINTS_COLOR = QColor(QtCore.Qt.red)
...
class BindRotateTool(ToolBase):
    ...
    def __init__(self, painter: MapCanvasPainter):
        super(BindRotateTool, self).__init__(painter)
        self.painter = painter
        self.active_pair: str = 'movable'  # либо 'base'
        self.pairs_dict = {'movable': [], 'base': []}
        self.colors_dict = {'movable': MOVABLE_POINTS_COLOR, 'base': BASE_POINTS_COLOR}
        ...
        self.rubbers_dict = {'movable': rli_line_rubber_band, 'base': map_line_rubber_band}
    ...
    def finish(self):
        self.finished.emit(
            [
                {
                    'start_point': _drawable_coords_to_dict(self.pairs_dict['movable'][0]),
                    'finish_point': _drawable_coords_to_dict(self.pairs_dict['movable'][1])
                },
                {
                    'start_point': _drawable_coords_to_dict(self.pairs_dict['base'][0]),
                    'finish_point': _drawable_coords_to_dict(self.pairs_dict['base'][1])
                }
            ]
        )
        self.drop()
    ...
    def drop(self):
        for point in self.pairs_dict['movable']:
            point.remove()
        self.pairs_dict['movable'].clear()

        for point in self.pairs_dict['base']:
            point.remove()
        self.pairs_dict['base'].clear()
    ...
    def keyPressEvent(self, e):
        if e.key() == QtCore.Qt.Key_Shift:
            if self.__points_pair_is_full():
                if self.active_pair == 'base':
                    self.finish()
                else:
                    self.active_pair = 'base'
    ...
    def __create_point(self, coordinates):
        ...
        drawable.style.label = f'{len(points_list)}'
        drawable.draw()
